...
import { ScheduleSearchProps, UserFullDataProps } from "../../@types";
...
interface LogEntry {
  ...
  changedBy: string;
  timestamp: Timestamp;
  changes: Changes<ScheduleSearchProps>; // A tipagem de changes será dinâmica
}
...
export default function ScheduleLogs({
  ...
  useEffect(() => {
    if (logsDb && logsDb.length > 0 && !logsDbLoading && !logsDbError) {
      ...
      setFilterSchoolLogs(logs.filter((log) => log.entityId === scheduleId));
    }
  }, [logsDb, logsDbLoading, logsDbError, scheduleId]);
        ...
        <table>
          <tbody>
            ...
            <tr className="flex gap-2">
              ...
              <td className="py-2">
                {handleDetails({ id: log.entityId, type: "schedules" })?.name}
              </td>
            </tr>
            <tr className="flex gap-2">
              <td className="py-2 font-bold">Início da Transição:</td>
              <td className="py-2">
                {
                  handleDetails({ id: log.entityId, type: "schedules" })
                    ?.transitionStart
                }
              </td>
            </tr>
            <tr className="flex gap-2">
              <td className="py-2 font-bold">Fim da Transição:</td>
              <td className="py-2">
                {
                  handleDetails({ id: log.entityId, type: "schedules" })
                    ?.transitionEnd
            ...
            <tr className="flex gap-2">
              <td className="py-2 font-bold">Início da Aula:</td>
              <td className="py-2">
                {
                  handleDetails({ id: log.entityId, type: "schedules" })
                    ?.classStart
            ...
            <tr className="flex gap-2">
              <td className="py-2 font-bold">Fim da Aula:</td>
              <td className="py-2">
                {
                  handleDetails({ id: log.entityId, type: "schedules" })
                    ?.classEnd
            ...
            <tr className="flex gap-2">
              <td className="py-2 font-bold">Saída:</td>
              <td className="py-2">
                {handleDetails({ id: log.entityId, type: "schedules" })?.exit}
              </td>
            </tr>
          </tbody>
        </table>
      ...
      return (
        <table>
          <tbody>
            ...
            {/* Verificando se a propriedade transitionStart existe dentro de changes */}
            {log.changes.transitionStart && (
              <>
                <tr className="flex gap-2">
                  <td className="py-2 font-bold" colSpan={2}>
                    Início da Transição
                ...
                {log.changes.transitionStart.before &&
                  log.changes.transitionStart.after && (
                    <tr className="flex gap-2 border-b border-klGreen-500">
                      <td className="py-2">Anterior:</td>
                      <td className="py-2 text-red-500">
                        {log.changes.transitionStart.before}
            ...
            {/* Verificando se a propriedade transitionEnd existe dentro de changes */}
            {log.changes.transitionEnd && (
              <>
                <tr className="flex gap-2">
                  <td className="py-2 font-bold" colSpan={2}>
                    Fim da Transição
                ...
                {log.changes.transitionEnd.before &&
                  log.changes.transitionEnd.after && (
                    <tr className="flex gap-2 border-b border-klGreen-500">
                      <td className="py-2">Anterior:</td>
                      <td className="py-2 text-red-500">
                        {log.changes.transitionEnd.before}
            ...
            {/* Verificando se a propriedade classStart existe dentro de changes */}
            {log.changes.classStart && (
              <>
                <tr className="flex gap-2">
                  <td className="py-2 font-bold" colSpan={2}>
                    Início da Aula
                ...
                {log.changes.classStart.before &&
                  log.changes.classStart.after && (
                    <tr className="flex gap-2 border-b border-klGreen-500">
                      <td className="py-2">Anterior:</td>
                      <td className="py-2 text-red-500">
                        {log.changes.classStart.before}
            ...
            {/* Verificando se a propriedade classEnd existe dentro de changes */}
            {log.changes.classEnd && (
              <>
                <tr className="flex gap-2">
                  <td className="py-2 font-bold" colSpan={2}>
                    Fim da Aula
                ...
                {log.changes.classEnd.before && log.changes.classEnd.after && (
                  <tr className="flex gap-2 border-b border-klGreen-500">
                    <td className="py-2">Anterior:</td>
                    <td className="py-2 text-red-500">
                      {log.changes.classEnd.before}
                    </td>

                    <td className="py-2">Atualizado:</td>
                    <td className="py-2 text-green-600">
                      {log.changes.classEnd.after}
                    </td>
                  </tr>
                )}
              </>
            )}
            {/* Verificando se a propriedade exit existe dentro de changes */}
            {log.changes.exit && (
              <>
                <tr className="flex gap-2">
                  <td className="py-2 font-bold" colSpan={2}>
                ...
                {log.changes.exit.before && log.changes.exit.after && (
                  <tr className="flex gap-2 border-b border-klGreen-500">
                    <td className="py-2">Anterior:</td>
                    <td className="py-2 text-red-500">
                      {log.changes.exit.before}
                    </td>

                    <td className="py-2">Atualizado:</td>
                    <td className="py-2 text-green-600">
                      {log.changes.exit.after}
                    </td>
                  </tr>
                )}
              </>

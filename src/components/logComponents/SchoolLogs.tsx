...
import { SchoolSearchProps, UserFullDataProps } from "../../@types";
...
interface LogEntry {
  ...
  changedBy: string;
  timestamp: Timestamp;
  changes: Changes<SchoolSearchProps>; // A tipagem de changes será dinâmica
}
// INITIALIZING FIRESTORE DB
const db = getFirestore(app);

interface SchoolLogsProps {
  schoolId: string;
  onClose: () => void; // Função para fechar o modal
}

export default function SchoolLogs({ schoolId, onClose }: SchoolLogsProps) {
  // GET GLOBAL DATA
  const {
    appUsersDb,
    schoolsDb,
  ...
  useEffect(() => {
    if (logsDb && logsDb.length > 0 && !logsDbLoading && !logsDbError) {
      ...
      setFilterSchoolLogs(logs.filter((log) => log.entityId === schoolId));
    }
  }, [logsDb, logsDbLoading, logsDbError, schoolId]);
        ...
        <table>
          <tbody>
            ...
            <tr className="flex gap-2">
              ...
              <td className="py-2">
                {handleDetails({ id: log.entityId, type: "schools" })?.name}
              </td>
            </tr>
          </tbody>
        </table>
